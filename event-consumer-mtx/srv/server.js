...
async function getCFInfo(appname) {
    try {
        // get app GUID
        let res1 = await httpClient.executeHttpRequest({ destinationName: 'event-consumer-mtx-cfapi' }, {
            method: 'GET',
            url: '/v3/apps?organization_guids=' + appEnv.app.organization_id + '&space_guids=' + appEnv.app.space_id + '&names=' + appname
        });
        // get domain GUID
        let res2 = await httpClient.executeHttpRequest({ destinationName: 'event-consumer-mtx-cfapi' }, {
            method: 'GET',
...
async function getDispatcherDestination(path) {
    try {
        const dest = await getDestination({ destinationName: 'Event-Dispatcher' });
        dest.originalProperties.destinationConfiguration.URL += path;
        dest.url += path;
        return dest;
    } catch (err) {
        console.log(err.stack);
...
async function registerTenant(operation, tenantId, subDomain, tenantURL) {
    try {
        const dispatcherDest = await getDispatcherDestination('/event-dispatcher/' + operation + 'Tenant');
        const payload = {
            tenant: {
                id: tenantId,
                subDomain: subDomain,
                url: tenantURL,
                source: (operation === 'register') ? 'new-tenant' : null
            }
        }
        const resp = await httpClient.executeHttpRequest(dispatcherDest, { method: 'POST', data: payload }, { fetchCsrfToken: false });
        return resp;
    } catch (err) {
        console.log(err.stack);
        return err.message;
...
cds.on('served', () => {
    ...
    provisioning.prepend(() => {
        ...
        provisioning.on('UPDATE', 'tenant', async (req, next) => {
            ...
            await registerTenant('register', req.data.subscribedTenantId, req.data.subscribedSubdomain, tenantURL);
            return tenantURL;
        });
        ...
        provisioning.on('DELETE', 'tenant', async (req, next) => {
            ...
            await registerTenant('unregister', req.data.subscribedTenantId, null, null);
            return req.data.subscribedTenantId;
        });
